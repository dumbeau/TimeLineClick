...
TimelineClick(images, yOffsets, hold=True){
	...
	;Determine if image paths are included in the s_SearchedImages array
	;If not, add them to the array
	static s_SearchedImages := []
	for k,image in images
	{
		if !TC_HasValue(s_SearchedImages, image) {
			;Verify the image file exists and can be read
			if (!FileExist(image))
			{
				msgbox, % "The image file cannot be read:`n" image
				Return
			}
			s_SearchedImages.push(image)
		}
